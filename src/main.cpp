...
#if defined(NDEBUG)
# error "Vakilcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf9eca1186646cf16873e70098d236757ddf62b3a082a222da9e05380b806377");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vakilcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vakilcoin Signed Message:\n";
...
{
    // Vakilcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Vakilcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Vakilcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Vakilcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vakilcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vakilcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7a9e0bb5893996bb39ed025ac214ead9ab13c8150635b4a3062d7025f234624c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Vakil Lab launches it's services across Globe";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0425345e78ba3699957e90ae4e2920b81a8e6f8cea95b4db25982de9d5b01a517c9cddffd4385228704668632f7fb8ec248d6b38e3fe28b18158d67ef694365c5e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637663209;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087195866;

        if (fTestNet)
        {
            block.nTime    = 1637663098;
            block.nNonce   = 385441118;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0395e311754549877113343aeaf67ae2dac587d83610492f35c0c74084cb3ec3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc7, 0xbc, 0xd3 }; // Vakilcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VakilcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VakilcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VakilcoinMiner(CWallet *pwallet)
{
    printf("VakilcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vakilcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VakilcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VakilcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VakilcoinMiner, pwallet));
}
